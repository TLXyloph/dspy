...
def test_callback_handlers_fire_inside_worker_threads():
    ...
    the handlers. (Full ACTIVE_CALL_ID call-ancestry transport is a separate concern; see the
    xfail test below.)
    """
...
@pytest.mark.xfail(
    reason="ACTIVE_CALL_ID call-ancestry does not transit to workers: ParallelExecutor uses a "
    "bare executor.submit without contextvars.copy_context. Tracked upstream via #10043.",
    strict=False,
)
